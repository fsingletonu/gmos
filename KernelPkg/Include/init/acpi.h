...
#define ACPI_SDT_HEADER_LEN sizeof(acpi_sdt_header_t)
...
extern RSDPDescriptor_t *g_rdsp;
...
} acpi_madt_local_apic_override_t;
#pragma pack(pop)
...
} aml_definition_block_header_t;
#pragma pack(pop)

#endif

#define ACPI_MCFG_DESC
#ifdef ACPI_MCFG_DESC
...
{
    acpi_sdt_header_t header; // 标准 ACPI 表头，签名 "MCFG"
    uint64_t reserved;        // 保留字段，必须为 0
} acpi_mcfg_t;
...
// MCFG 表后面的“配置空间基址描述符”（每个 PCIe 段一个）
typedef struct acpi_mcfg_alloc
{
    uint64_t base_address;      // 配置空间的 64 位物理基址
    uint16_t pci_segment_group; // PCI 段组号（通常为 0）
    uint8_t start_bus_number;   // 本区域起始总线号
    uint8_t end_bus_number;     // 本区域结束总线号
...
phys_addr seek_acpi_signature(char signature[4]);
